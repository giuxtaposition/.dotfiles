...
export class SwayService extends Service {
  static {
    Service.register(
      this,
      {},
  ...
  msg(payload: string) {
    this._send(PAYLOAD_TYPE.MESSAGE_RUN_COMMAND, payload);
  }

  constructor() {
    if (!SIS) console.error("Sway is not running");
    super();
  ...
  private _handleTreeMessage(node: Node) {
    switch (node.type) {
      ...
      case "output":
        ...
        if (node.active) this._active.monitor.update(node.id, node.name);
        node.nodes.map((n) => this._handleTreeMessage(n));
      ...
      case "workspace":
        ...
        // eslint-disable-next-line no-case-declarations
        const hasFocusedChild: (n: Node) => boolean = (n: Node) =>
          n.nodes.some((c) => c.focused || hasFocusedChild(c));
...
export const swayService = new SwayService();
